...
def make_query_str(args, cat_lst, keyword_lst):
    if args.start_time != '' and args.end_time != '':
        yesterday_str = args.start_time
        today_str = args.end_time
    else:
...
def get_daily_arxiv_papers(args, cat_lst, keyword_lst, keywords_bin, paper_set, max_results=100):
    query_results = make_query_str(args, cat_lst, keyword_lst)
...
def make_md_and_html(args, keyword_lst, keywords_bin):
    # now = datetime.datetime.utcnow()
    now = args.start_time[:4] + '-' + args.start_time[4:6] +'-'+ args.start_time[6:]
